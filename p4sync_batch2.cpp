...
vector<pair<string, int>> work2;
atomic<int> work2_total = 0;
atomic<int> work2_index = 0;
mutex work2_mu;

bool work2_done()
{
   lock_guard<mutex> guard(work2_mu);
   return work2.empty();
}

bool try_get_work2(string& task, int& size)
{
   lock_guard<mutex> guard(work2_mu);
   if (work2.empty()) return false;
   const auto& [f, sz] = work2.back();
   task = f;
   size = sz;
   work2_index++;
   work2.pop_back();
...
void expect(char*& p, cstr t)
{
   for (; *p && *t && *p==*t; p++, t++);
   if (*t != 0) throw exception("Error retrieving tag");
}
...
int main(int argc, char* argv[])
{
   ...
   for (string dir: dirs_to_sync) // iterating by value intentionally
   {
   ...
   #if BATCH_SYNC
      static const string fileSizeTag = "... fileSize"s;

      string temp_path = get_temp_path();
      sysfmt("p4 fstat -Ol -T \"depotFile, fileSize\" \"%s/...\" > %s", *dir, *temp_path);
      ...
      while (*p)
      {
         ...
         *p++ = 0;
         ...
         work2.emplace_back(name, sz);
      }

      work2_total = (int)work2.size();
      work2_index = 0;
   #else
      string temp_path = get_temp_path();
   ...
   for (int i=0; i<num_threads; i++)
   {
      workers.emplace_back([](int id)
      {
         ...
         while (!done.load() && !critical.load())
         {
            ...
            int osize = 0;
            char buf[1024];
            while (batch < batch_size && try_get_work2(opath, osize))
            {
               batch += osize;
               sprintf(buf, "\"%s#head\" ", *opath);
               tosync += buf;
            }

            string temp_path = get_temp_path();
            if (batch < KB)
               sprintf(buf, "%lld bytes", batch);
            else if (batch < MB)
               sprintf(buf, "%.2f KB", batch/(float)KB);
            else if (batch < GB)
               sprintf(buf, "%.2f MB", batch/(float)MB);
            else
               sprintf(buf, "%.2f GB", batch/(float)GB);

            print(gray, "Syncing batch: %s\n", buf);
            cmd = "p4 -s sync "s + tosync + " > "s + temp_path;
            system(*cmd);
            bool error = false;
            vector<string> toclobber;
               ...
               for (const auto& [tag, color, iserror]: msg_types)
               {
                  if (prefix == tag)
                  {
                     ...
                     if (iserror)
                     {
                        ...
                        {
                           cstr path = strstr(*body, "//");
                           if (path)
                              toclobber.push_back(path); // VERIFY
                        }
                     }
                     break;
                  }
            ...
            for (const string& path: toclobber)
            {
...
#if BATCH_SYNC
   while (!work2_done() && !critical.load())
   {
